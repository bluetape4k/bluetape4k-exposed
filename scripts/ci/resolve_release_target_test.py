...
class ReleaseWorkflowPolicyTest(unittest.TestCase):
    def test_release_jobs_checkout_and_verify_one_immutable_target(self) -> None:
        ...
        self.assertGreaterEqual(workflow.count("resolve_release_target.py verify"), 3)
        self.assertIn("Source commit", workflow)

    def test_ci_status_requires_release_workflow_contract(self) -> None:
        workflow = CI_WORKFLOW.read_text(encoding="utf-8")
        ci_status = workflow.split("  ci-status:\n", maxsplit=1)[1]

        self.assertIn("      - release-workflow-contract\n", ci_status)
