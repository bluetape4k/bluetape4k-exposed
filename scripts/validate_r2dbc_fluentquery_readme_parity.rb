module R2dbcCoroutineFluentQueryReadmeParity
  START_MARKER = "<!-- r2dbc-coroutine-fluent-query:START -->"
  END_MARKER = "<!-- r2dbc-coroutine-fluent-query:END -->"
  REQUIRED_CONTRACT_KEYS = %w[
    ...
    outer-transaction
    database-selection
    nested-transactions-rejected
    closed-projection
    open-projection-rejected
    matcher-projection-matrix
    find-one-cardinality
    first-one-all-page-slice-count-exists
    error-taxonomy
    callback-scope
    cancellation
    streaming-retry-no-duplicate
    terminal-retry-delegated
  ].freeze
  ...
  class Validator
    ...
    def marked_section(source, path)
      ...
      if starts != 1 || ends != 1
        errors << "#{path}: expected exactly one R2DBC coroutine FluentQuery marker pair"
        return
      end
      ...
      if end_index <= start_index
        errors << "#{path}: R2DBC coroutine FluentQuery end marker precedes start marker"
        return
      end
    ...
    def compare(english, korean)
      {
        ...
        errors << "R2DBC coroutine FluentQuery README #{label} differ" unless left == right
      end
    end
  end
end
...
if $PROGRAM_NAME == __FILE__
  ...
  errors = R2dbcCoroutineFluentQueryReadmeParity::Validator.new(ARGV[0], ARGV[1]).validate
  if errors.empty?
    puts "R2DBC coroutine FluentQuery README parity is aligned"
  else
    warn errors.join("\n")
    exit 1
  end
