...
범위: `1.12.1` publication metadata 교정 릴리스.
...
downstream consumer 검증을 release gate로 고정합니다.

## 검증 기준

- `baseVersion=1.12.1`, 빈 `snapshotVersion`, immutable catalog commit을 사용합니다.
- 35개 publication의 POM과 Gradle Module Metadata를 모두 검사합니다.
- 34개 일반 artifact와 1개 test fixture를 격리된 로컬 repository에 게시한 뒤 compile/runtime
  dependency resolution을 검증합니다.
- exact-head CI, Nightly, Snapshot, signed tag, Maven Central publication 순서로 진행합니다.
...
- release candidate는 `catalog/2026-08-06-02` 준비 commit을 사용하며, 공개 릴리스 완료 후
