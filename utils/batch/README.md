...
again, and rerun the same migration file from the beginning.
...
`completeJobExecution` and `completeStepExecution` accept only terminal statuses: `COMPLETED`, `COMPLETED_WITH_SKIPS`, `FAILED`, and `STOPPED`. `STARTING` and `RUNNING` are rejected before persistence with `BatchCompletionStatusException`, which carries the rejected status. Exhausted bounded recovery raises `BatchRepositoryRecoveryExhaustedException` with a 16-character Base58 correlation ID.
