...
tasks.register("verifyProvenance") {
    doLast {
        check(sourceHead.isNotBlank()) { "ISSUE731_SOURCE_HEAD must identify the publication checkout" }
        check(sourceHead == expectedHead) { "published sourceHead=$sourceHead does not match expectedHead=$expectedHead" }
    }
}
