...
import io.bluetape4k.batch.CheckpointJson as StableCheckpointJson
...
class CheckpointJsonTest {
    ...
    private val sut: StableCheckpointJson = StableCheckpointJson.jackson3()
    ...
    fun `registered data class round-trip`() {
        val sut = StableCheckpointJson.jackson3(KeyState::class)
    ...
    fun `TypedCheckpoint 봉투 포함 검증 - className이 JSON에 있음`() {
        ...
        json shouldBeEqualTo """{"className":"java.lang.Long","payload":"42"}"""
    }
    ...
    fun `jackson3 팩토리 - tools_jackson이 있으면 성공`() {
        ...
        val result = runCatching { StableCheckpointJson.jackson3() }
    ...
    fun `jackson3 팩토리 - 반환 타입이 CheckpointJson`() {
        StableCheckpointJson.jackson3() shouldBeInstanceOf StableCheckpointJson::class
    }
    ...
    @Test
    fun `이전 internal CheckpointJson 브리지가 stable 타입으로 위임`() {
        val legacy = CheckpointJson.jackson3()
        val restored = legacy.read(legacy.write(42L))
