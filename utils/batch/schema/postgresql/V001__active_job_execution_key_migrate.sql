...
ALTER TABLE batch_job_execution DROP CONSTRAINT IF EXISTS batch_job_exec_status_active_key_chk;
...
ALTER TABLE batch_job_execution DROP CONSTRAINT IF EXISTS batch_job_exec_active_params_hash_v2_chk;
ALTER TABLE batch_job_execution ADD CONSTRAINT batch_job_exec_active_params_hash_v2_chk CHECK (
    active_key IS NULL
    OR params_hash = ''
    OR params_hash ~ '^[0-9a-f]{64}$'
);
