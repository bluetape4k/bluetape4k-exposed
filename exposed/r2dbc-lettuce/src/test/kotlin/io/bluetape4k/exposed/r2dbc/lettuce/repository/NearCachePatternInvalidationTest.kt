...
class NearCachePatternInvalidationTest: AbstractR2dbcLettuceTest() {
    ...
    fun `0 이하 count는 Redis에 접근하기 전에 거부한다`() = runSuspendIO {
        val repository = R2dbcUserLettuceRepository(mockk())
    ...
    fun `backing cache failure는 호출자에게 전파한다`() = runSuspendIO {
        val repository = R2dbcUserLettuceRepository(
