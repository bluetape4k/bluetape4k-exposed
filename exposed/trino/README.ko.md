...
![Trino JDBC compatibility boundary diagram](../../docs/images/readme-diagrams/exposed-trino-diagram-01.png)

### Flow materialization 계약

![Trino Flow materialization contract diagram](../../docs/images/readme-diagrams/exposed-trino-flow-02.png)
...
대용량 결과셋이나 optimizer 민감 workload에서는 `TrinoConnectionOptions`로 Trino JDBC property를 타입 안전하게 전달할 수 있습니다. 기존 `connect`
오버로드는 그대로 유지됩니다.
...
실제 pushdown 지원은 connector별로 다릅니다. 대상 catalog에서 `EXPLAIN`으로 predicate, projection, aggregation, top-N, limit pushdown 같은 안정적인 신호만 확인하고, 테스트에서는 전체 plan snapshot 비교를 피하세요.
...
`pagedQueryFlow`를 사용합니다. 각 page는 별도 Exposed 트랜잭션 안에서 로드되고, 트랜잭션이 닫힌 뒤 emit 됩니다.
...
- `pageSize`는 애플리케이션 쪽 materialize 크기를 제한합니다. Trino JDBC 처리량 튜닝은 대용량 결과 전송을 위한 Trino spooling protocol을 포함해 드라이버/클러스터 프로토콜 영역입니다.
...
대상 Trino catalog가 `INSERT`를 지원하고, Exposed `batchInsert`에 명시적인 client-side chunking을 적용하려면 `trinoBatchInsert`를 사용합니다.
...
- Trino는 `INSERT INTO ... query`와 multi-row `VALUES` 문법을 지원하지만, 실제 쓰기 지원 여부는 커넥터별로 다릅니다.
- `trinoBatchInsert`는 Exposed JDBC `batchInsert`를 제한된 chunk로 감싼 helper이며, Trino connector 전용 bulk-loader protocol은 아닙니다.
- generated keys는 안정적인 Trino 쓰기 계약이 아니므로
  `shouldReturnGeneratedValues` 기본값은 `false`입니다.
- 후속 chunk가 실패하면 앞선 chunk는 이미 반영되어 있을 수 있습니다. 이 모듈은 Trino 쓰기에 대해 rollback 또는 all-or-nothing 의미를 보장하지 않습니다.
- JDBC connector의 `write.batch-size` 같은 connector-side write tuning은 Trino catalog 설정 영역입니다.
