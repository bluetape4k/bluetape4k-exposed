...
class ExposedEntityMapLoaderParallelTest: AbstractExposedTest() {
    ...
    private object LoaderTable: LongIdTable("redisson_parallel_loader_test") {
    ...
    @Test
    fun `parallel key enumeration matches Redisson synchronous loader and preserves range order`() {
    ...
    @Test
    fun `parallel key enumeration keeps empty range and Redisson callback contracts`() {
        withTables(TestDB.H2, LoaderTable) {
            commit()
            val loader = ExposedEntityMapLoader(
                entityTable = LoaderTable,
