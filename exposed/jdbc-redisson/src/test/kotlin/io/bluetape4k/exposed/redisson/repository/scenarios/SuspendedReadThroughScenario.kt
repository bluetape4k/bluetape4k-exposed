...
interface SuspendedReadThroughScenario<ID: Any, E: java.io.Serializable>: SuspendedCacheTestScenario<ID, E> {
    ...
    fun `getAll - batchSize 는 0보다 커야 한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
            assertFailsWith<IllegalArgumentException> {
                (repository as AbstractSuspendedJdbcRedissonRepository<ID, E>)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `countFromDb - DB에서 전체 레코드 수를 직접 조회한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `findAll - limit, offset 파라미터로 페이지 조회한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
