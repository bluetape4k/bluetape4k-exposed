...
interface SuspendedReadThroughScenario<ID: Any, E: java.io.Serializable>:
    ...
    fun `get - 캐시 미스 시 DB에서 Read-through로 값을 로드한다`(testDB: TestDB) = runSuspendIO {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `get - DB에 없는 ID는 null을 반환한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `getAll - 여러 ID 일괄 조회 시 캐시 미스 키를 DB에서 Read-through한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `getAll - 존재하지 않는 ID는 결과에 포함되지 않는다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `clear - 캐시를 비운 후 재조회하면 DB에서 다시 Read-through한다`(testDB: TestDB) = runSuspendIO {
        withSuspendedEntityTable(testDB) {
            val id = getExistingId()
            repository.get(id)
            repository.clear()
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `invalidate - 캐시에만 저장된 엔티티를 삭제하면 get은 null을 반환한다`(testDB: TestDB) = runSuspendIO {
