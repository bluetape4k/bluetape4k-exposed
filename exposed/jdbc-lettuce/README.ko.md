...
- **MapLoader / MapWriter**: repository loaded-map 연동을 위한 Exposed 기반 구현체
    ...
    - 각 page는 `batchSize`만 materialize하므로 JDBC query와 connection이 page 경계를 넘어 벗어나지 않습니다. suspended loader는 하나의 `suspendedTransactionAsync` 안에서 page loop를 실행하며, ambient caller-owned Exposed transaction의 소유권은 호출자에게 남습니다
