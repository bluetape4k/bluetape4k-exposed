...
class SuspendedQueryTest: AbstractExposedTest() {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `batchSize 가 0 이하이면 예외를 던진다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            assertFailsWith<IllegalArgumentException> {
                ProductTable
                    .select(ProductTable.id)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `기존 where 조건을 유지한 채 배치 조회한다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `추가 where 조건을 기존 query 조건과 AND 로 결합한다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            listOf(10, 20, 30, 40, 50).forEachIndexed { index, price ->
                ProductTable.insert {
                    it[name] = "product-$index"
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `첫 번째 선택 컬럼이 Int Long 이 아니면 예외를 던진다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            ProductTable.insert {
                it[name] = "sample"
                it[price] = 100
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `빈 테이블에서 배치 조회하면 빈 리스트를 반환한다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            val batches =
                ProductTable
                    .select(ProductTable.id)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `수동 LIMIT 이 설정된 query 에서 배치 조회하면 예외를 던진다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            assertFailsWith<IllegalArgumentException> {
                ProductTable
                    .select(ProductTable.id)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `수동 ORDER BY 가 설정된 query 에서 배치 조회하면 예외를 던진다`(testDB: TestDB) = runSuspendIO {
        withTablesSuspending(testDB, ProductTable) {
            assertFailsWith<IllegalArgumentException> {
                ProductTable
                    .select(ProductTable.id)
