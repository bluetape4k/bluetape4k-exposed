...
class R2dbcRepositoryMultiRowValuesTest: AbstractExposedR2dbcTest() {
    ...
    fun `native multi-row 부분 충돌의 반환 행 누락을 분리 재현한다`(testDB: TestDB) = runSuspendIO {
    ...
    private object Rows: LongIdTable("r2dbc_repository_multi_rows") {
    ...
    private object Repository: LongR2dbcRepository<Record> {
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `기존 positional 호출과 명시적 false는 행별 parameter set을 유지한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `multi-row는 nullable 데이터와 생성 ID 순서를 하나의 parameter set으로 반환한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
            val observer = InsertLog()
            addLogger(observer)
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `빈 입력은 바인더와 INSERT를 실행하지 않고 단일 Sequence도 지원한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
            val observer = InsertLog()
            addLogger(observer)
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `행 수 추정 한도까지 저장하고 초과 입력은 바인더 전에 거부한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `무한 Sequence는 한도 다음 행까지만 소비하고 선행 쓰기를 보존한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `생성 값 요청을 끈 명시 ID 데이터는 별도 SELECT로 확인한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
    ...
    @ParameterizedTest
    @MethodSource("multiRowDialects")
    fun `multi-row와 ignore 조합은 입력 순회와 INSERT 전에 거부한다`(testDB: TestDB) = runSuspendIO {
        withTables(testDB, Rows) {
