...
class R2dbcFixtureConcurrencyTest {
    private fun fixture(key: String) = r2dbcTestDbFixture(key, { configure ->
    ...
    fun `종료된 진입 토큰은 후속 호출을 거부하지 않는다`() = runSuspendIO {
        ...
        withDb(fixture) { inherited = kotlinx.coroutines.currentCoroutineContext().minusKey(kotlinx.coroutines.Job) }
        withTimeout(5_000) { withContext(inherited) { withDb(fixture) {} } }
    ...
    fun `permit 획득 후 생성 callback에서 취소되어도 반환한다`() = runSuspendIO {
        coroutineScope {
            ...
            var bodyRan = false
            val fixture = r2dbcTestDbFixture("r2dbc_acquired_cancel", { configure ->
                job.cancel()
                R2dbcDatabase.connect(databaseConfig = R2dbcDatabaseConfig {
            ...
            job = launch(start = CoroutineStart.LAZY) { withDb(fixture) { bodyRan = true } }
            job.start()
            job.join()
    ...
    fun `대기자는 FIFO로 진행하고 다른 fixture는 막지 않는다`() = runSuspendIO {
        withTimeout(5_000) {
            coroutineScope {
                ...
                val first = launch(start = CoroutineStart.UNDISPATCHED) { withDb(fixture) { order.add(1) } }
                val second = launch(start = CoroutineStart.UNDISPATCHED) { withDb(fixture) { order.add(2) } }
                withDb(other) { order.isEmpty() shouldBeEqualTo true }
    ...
    fun `대기 취소 뒤 후속 호출이 실행된다`() = runSuspendIO {
        withTimeout(5_000) {
            coroutineScope {
                ...
                val first = launch { withDb(fixture) { entered.complete(Unit); awaitCancellation() } }
                entered.await()
                var cancelledBody = false
                val waiting = launch(start = CoroutineStart.UNDISPATCHED) {
                    withDb(fixture) { cancelledBody = true }
                }
                waiting.cancelAndJoin()
                first.cancelAndJoin()
                withDb(fixture) { cancelledBody shouldBeEqualTo false }
            }
        }
    }
    ...
    fun `dispatcher와 자식 coroutine에서 활성 중첩을 거부한다`() = runSuspendIO {
        ...
        withTimeout(5_000) {
            withDb(fixture) {
                ...
                coroutineScope {
                    launch { assertFailsWith<IllegalStateException> { withDb(fixture) {} } }.join()
                }
            }
            withDb(fixture) {}
        }
