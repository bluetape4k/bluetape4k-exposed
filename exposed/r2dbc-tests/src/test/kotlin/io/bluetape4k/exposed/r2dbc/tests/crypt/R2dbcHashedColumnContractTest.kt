package io.bluetape4k.exposed.r2dbc.tests.crypt
...
class R2dbcHashedColumnContractTest {
    ...
    private class Secrets(hasher: Hasher): Table("crypt_contract_r2dbc") {
        val id = integer("id")
    ...
    private object RawSecrets: Table("crypt_contract_r2dbc") {
        val secret = varchar("secret", 512)
    }

    @ParameterizedTest
    @ValueSource(strings = ["bcrypt", "argon2", "pbkdf2", "scrypt"])
    fun `각 알고리즘의 저장 조회 nullable 재저장은 평문을 보존하지 않는다`(algorithm: String) = runSuspendIO {
    ...
    @Test
    fun `custom hasher는 최초 변환에만 호출되고 재저장에서는 호출되지 않는다`() = runSuspendIO {
        var calls = 0
    ...
    fun `인증 성공 후에만 비용 또는 알고리즘을 승격한다`(changeAlgorithm: Boolean) = runSuspendIO {
