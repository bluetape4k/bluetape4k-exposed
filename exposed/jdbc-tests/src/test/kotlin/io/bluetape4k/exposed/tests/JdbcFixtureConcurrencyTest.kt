...
class JdbcFixtureConcurrencyTest {
    private fun fixture(key: String) = jdbcTestDbFixture(key, { configure ->
        Database.connect("jdbc:h2:mem:$key;DB_CLOSE_DELAY=-1", "org.h2.Driver",
    ...
    fun `blocking과 suspend 대기자는 FIFO로 진행하고 다른 fixture는 막지 않는다`() = runSuspendIO {
        withTimeout(5_000) {
            coroutineScope {
                ...
                try {
                    val first = launch { withDb(fixture) { order.add(1) } }
                    await.untilSuspending { fixture.semaphore.queueLength == 1 }
                    val second = launch { withDbSuspending(fixture) { order.add(2) } }
                    await.untilSuspending { fixture.semaphore.queueLength == 2 }
                    withDbSuspending(other) { order.isEmpty() shouldBeEqualTo true }
    ...
    fun `종료된 진입 토큰을 상속해도 후속 호출을 거부하지 않는다`() = runSuspendIO {
        ...
        withDbSuspending(fixture) { inherited = currentCoroutineContext().minusKey(Job) }
        withTimeout(5_000) {
            withContext(inherited) { withDbSuspending(fixture) {} }
        }
        fixture.semaphore.availablePermits() shouldBeEqualTo 1
    }
    ...
    fun `permit 획득 후 wrapper 생성 중 취소해도 본문 없이 permit을 반환한다`() = runSuspendIO {
        coroutineScope {
            ...
            var bodyRan = false
            val fixture = jdbcTestDbFixture("jdbc_acquired_cancel", { configure ->
                job.cancel()
                Database.connect("jdbc:h2:mem:jdbc_acquired_cancel;DB_CLOSE_DELAY=-1", "org.h2.Driver",
                                 databaseConfig = DatabaseConfig { configure() })
            })
            job = launch(start = CoroutineStart.LAZY) { withDbSuspending(fixture) { bodyRan = true } }
            job.start()
            job.join()
    ...
    fun `suspend 대기 취소 뒤 permit을 후속 호출에 반환한다`() = runSuspendIO {
        withTimeout(5_000) {
            coroutineScope {
                ...
                val first = launch {
                    withDbSuspending(fixture) { entered.complete(Unit); awaitCancellation() }
                }
                entered.await()
                ...
                val waiting = launch {
                    ...
                    withDbSuspending(fixture) { cancelledBody = true }
                }
                waitingStarted.await()
                await.untilSuspending { fixture.semaphore.hasQueuedThreads() }
                waiting.cancelAndJoin()
                first.cancelAndJoin()
                withDbSuspending(fixture) { cancelledBody shouldBeEqualTo false }
            }
        }
    }
    ...
    fun `suspend에서 blocking과 dispatcher 전환 중첩은 즉시 거부한다`() = runSuspendIO {
        ...
        withTimeout(5_000) {
            withDbSuspending(fixture) {
                assertFailsWith<IllegalStateException> { withDb(fixture) {} }
                withContext(Dispatchers.Default) {
                    assertFailsWith<IllegalStateException> { withDbSuspending(fixture) {} }
                }
            }
            withDbSuspending(fixture) {}
        }
    ...
    fun `blocking 중첩은 거부하고 다른 fixture는 실행한다`() {
        ...
        withDb(first) {
            assertFailsWith<IllegalStateException> { withDb(first) {} }
            withDb(other) { currentJdbcTestDbFixture shouldBeSameInstanceAs other }
