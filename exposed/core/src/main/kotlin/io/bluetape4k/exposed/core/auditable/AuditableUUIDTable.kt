 ...
 *
 * ## 동작/계약
 * - `id`는 `javaUUID` 타입이며 INSERT 시 `Uuid.V7.nextId()`로 client-side 기본값이 생성됩니다.
...
abstract class AuditableUUIDTable(
     ...
     * INSERT 시 `Uuid.V7.nextId()`로 client-side에서 자동 생성됩니다.
     */
    final override val id: Column<EntityID<UUID>> =
        javaUUID(columnName).clientDefault { Uuid.V7.nextId() }.entityId()
