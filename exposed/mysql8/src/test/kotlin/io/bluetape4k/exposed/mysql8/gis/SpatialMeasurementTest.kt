...
class SpatialMeasurementTest: AbstractMySqlGisTest() {
    ...
    object PointMeasureTable: LongIdTable("measure_points") {
    ...
    // Polygon 테이블 (넓이 측정용)
    object PolygonMeasureTable: LongIdTable("measure_polygons") {
        val name = varchar("name", 255)
        val area = geoPolygon("area")
    }

    // LineString 테이블 (길이 측정용)
    object LineMeasureTable: LongIdTable("measure_lines") {
        val name = varchar("name", 255)
    ...
    fun `ST_Distance - 서울-수원 거리 약 30km`() {
        withGeoTables(PointMeasureTable) {
            PointMeasureTable.insert {
                it[name] = "서울-수원"
                it[pointA] = wgs84Point(126.9780, 37.5665)  // 서울
                it[pointB] = wgs84Point(127.0000, 37.2636)  // 수원
            }

            val distanceExpr = PointMeasureTable.pointA.stDistance(PointMeasureTable.pointB)
            val distance = PointMeasureTable.select(distanceExpr).single()[distanceExpr]
    ...
    fun `ST_Distance_Sphere - 서울-부산 거리 약 325km`() {
        withGeoTables(PointMeasureTable) {
            ...
            val distanceExpr = PointMeasureTable.pointA.stDistanceSphere(PointMeasureTable.pointB)
            val distance = PointMeasureTable.select(distanceExpr).single()[distanceExpr]
    ...
    fun `ST_Area - 큰 폴리곤이 작은 폴리곤보다 넓이가 크다`() {
        withGeoTables(PolygonMeasureTable) {
            PolygonMeasureTable.insert {
                it[name] = "large"
            ...
            val areaExpr = PolygonMeasureTable.area.stArea()
            val rows = PolygonMeasureTable.select(PolygonMeasureTable.name, areaExpr)
                .orderBy(areaExpr, SortOrder.ASC)
                .toList()

            // 작은 것(small)이 먼저 정렬됨
            rows[0][PolygonMeasureTable.name] shouldBeEqualTo "small"
            rows[1][PolygonMeasureTable.name] shouldBeEqualTo "large"
        }
    }

    @Test
    fun `ST_Length - LineString 길이 측정`() {
        withGeoTables(LineMeasureTable) {
            LineMeasureTable.insert {
                it[name] = "서울-수원"
            ...
            val lengthExpr = LineMeasureTable.path.stLength()
            val length = LineMeasureTable.select(lengthExpr).single()[lengthExpr]
