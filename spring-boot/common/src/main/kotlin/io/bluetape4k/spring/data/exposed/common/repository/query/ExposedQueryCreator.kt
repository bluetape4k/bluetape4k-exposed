...
/** Spring Data [PartTree]를 Exposed [Op] 조건으로 변환합니다. */
class ExposedQueryCreator(
    ...
    override fun create(part: Part, iterator: Iterator<Any>): Op<Boolean> = buildCondition(part, iterator)

    override fun and(part: Part, base: Op<Boolean>, iterator: Iterator<Any>): Op<Boolean> =
        base.and(buildCondition(part, iterator))

    override fun or(base: Op<Boolean>, criteria: Op<Boolean>): Op<Boolean> = base.or(criteria)

    override fun complete(criteria: Op<Boolean>?, _sort: Sort): Op<Boolean> = criteria ?: Op.TRUE

    @Suppress("UNCHECKED_CAST", "CyclomaticComplexMethod")
    private fun buildCondition(part: Part, iterator: Iterator<Any>): Op<Boolean> {
        val columnName = toSnakeCase(part.property.segment)
