...
class SimpleExposedR2dbcRepositoryRetryTest: AbstractExposedR2dbcRepositoryTest() {
    ...
    private fun configuredRetryMaxAttempts(): Int =
        System.getenv("BLUETAPE_R2DBC_SAVE_ALL_MAX_ATTEMPTS")?.let { value ->
            value.toIntOrNull()?.also {
                require(it > 0) { "BLUETAPE_R2DBC_SAVE_ALL_MAX_ATTEMPTS must be positive" }
            } ?: error("BLUETAPE_R2DBC_SAVE_ALL_MAX_ATTEMPTS must be an integer")
        } ?: 2
    ...
    fun `saveAll with Flow retries a transient failure and emits one successful attempt`(testDB: TestDB) =
        runSuspendIO {
            Assumptions.assumeTrue { testDB in setOf(TestDB.H2, TestDB.POSTGRESQL) }

            withRetryFaultDatabase(testDB, maxAttempts = configuredRetryMaxAttempts()) { faultFactory ->
                ...
                faultFactory.rollbackCount.get() shouldBeEqualTo 2
                faultFactory.closeCount.get() shouldBeEqualTo 2
                collectionCount.get() shouldBeEqualTo 2
                commitCountsAtEmission shouldBeEqualTo listOf(2, 2)
    ...
    @ParameterizedTest
    @MethodSource(AbstractExposedR2dbcTest.ENABLE_DIALECTS_METHOD)
    fun `saveAll with Iterable retries a transient failure and emits one successful attempt`(testDB: TestDB) =
        runSuspendIO {
            Assumptions.assumeTrue { testDB in setOf(TestDB.H2, TestDB.POSTGRESQL) }
