# Lessons Learned - Issue #288 R2DBC Declared Query Transaction Boundary (2026-06-23)
...
선언형 쿼리 테스트는 두 축을 모두 가져야 한다.

- active transaction 내부에서 미커밋 row가 보이는지
- active transaction 밖에서도 repository method가 자체 transaction으로 조회되는지

## L2: transaction 밖 테스트는 Exposed 기본 DB 선택을 명시해야 한다

### 문제

`withTables(testDB, ..., dropTables = false)`로 데이터를 만든 뒤 transaction 밖에서 `suspendTransaction {}`을 열면, Exposed는 active transaction이 없으므로 `TransactionManager.defaultDatabase` 또는 primary database를 사용한다. multi-dialect 테스트에서는 이 기본 DB가 현재 parameter의 `testDB`와 달라질 수 있다.

### 교훈

transaction 밖 동작을 검증하는 multi-dialect 테스트는 테스트가 만든 `testDB.db`를 `TransactionManager.defaultDatabase`로 임시 지정하고 반드시 `finally`에서 복원해야 한다. 그래야 테스트가 실제 production fallback 경로를 검증하면서도 DB 선택이 deterministic하다.
