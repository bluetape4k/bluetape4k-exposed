...
기준 커밋은 `d5fb9602491ad64811bffcda227e3b2deecf9eea`, 작업 브랜치는 `feat/issue-857-clickhouse-streaming`이다. 새 의존성, 다른 DB 모듈 변경, 기존 API 제거, PR 생성·머지·Full Nightly 실행은 범위 밖이다. 중앙 매뉴얼 수정은 소유 저장소의 별도 작업 범위로 관리한다.
...
현재 상태는 `PENDING`이다. 테스트 통과·실측·구현 완료를 주장하지 않는다. 명세 검토 후 실행 계획을 작성하며, PR 생성과 머지는 별도 승인 경계다.
