...
- First accepted work transitions `IDLE -> RUNNING`. Expected `close()` transitions `IDLE|RUNNING -> DRAINING` before the channel closes. Confirmed normal drain completion transitions `DRAINING -> STOPPED`; expected scope cancellation after `STOPPED` does not change it. An uncaught terminal failure or cancellation before or during draining transitions to `FAILED`. If the bounded close wait expires, `close()` sets `FAILED` before cancelling the scope and never leaves the returned repository indefinitely in `DRAINING`; a late completion cannot overwrite `FAILED` with `STOPPED`.
...
Meters and their stable, thread-safe state holders are created once when routes are installed and reused for every request. Four gauges per contributor hold one immutable sample behind an atomic reference; fields that do not apply, have not succeeded yet, or belong to an error/timeout result publish `NaN` rather than a stale value. A monotonic generation is claimed per contributor only when its probe begins or it receives a synthetic budget-exhausted timeout. Only the newest claimed generation for that contributor may publish, so a late completion or cancellation from an older attempt cannot overwrite a newer sample; a newer request cancelled before reaching that contributor does not suppress the older in-flight result. Parent cancellation sets the active contributor's gauges to `NaN`; contributors not yet invoked retain their last completed sample. A successful probe publishes all four fields from its one sanitized snapshot. Timers use four finite outcome series. The upper bound is eight series per contributor, or 128 cache series per route installation at the 16-contributor limit, and repeated or concurrent requests cannot register more meters.

Before registering any cache meter, installation rejects an existing meter with the same library-owned name plus `component` and `kind` identity in the caller's `MeterRegistry`; registration is all-or-nothing so a rejected install leaves no partial meters. Multiple route installations with distinct identities add at most 128 series each, so the application-wide bound is `128 * cache-route-installation-count`. Documentation recommends one Exposed readiness route per application/registry. The library does not add a global registry or silently share a state holder across routes.
...
2. **Probe throws an ordinary exception**: return `DOWN`, emit `error`, and discard message/cause/stack details at the HTTP and metric boundaries.
3. **Coroutine cancellation**: distinguish parent/request cancellation from the cache-phase timeout, emit exactly one `cancelled` outcome when a registry is present, and rethrow without an HTTP failure detail.
...
- Prove one active cache timeout emits one `timeout` and zero `cancelled` outcomes, skipped contributors emit no timer and expose `NaN` gauges, and parent cancellation emits one `cancelled`, rethrows, and emits no HTTP failure detail.
...
- Verify timer tags and measurement meters use only the allowed finite fields, the exact meter names/base units/descriptions, `CacheWriteMode` is not a tag, repeated and concurrent probes keep meter count constant, per-contributor generation ordering handles a newer request that never reaches a contributor, thread-safe gauge updates do not throw, error/timeout gauges become `NaN`, cancellation clears only the active contributor's gauges, and the maximum is 128 cache series per route installation.
- Verify duplicate meter identity installation fails before any meter is added, distinct route installations follow the application-wide series formula, and no route silently binds gauges to another route's state holder.
- Verify snapshot sampling remains read-only and bounded while producers and drainers run concurrently.
- Verify the all-backend planning budget with JDBC, R2DBC, and cache contributors configured, including a saturated blocking dispatcher where a JDBC query starts near `R` and then consumes `J_effective`, plus an orchestrator timeout example with deployment margin.
...
- Add a Ktor authentication test proving unauthenticated denial and authenticated readiness access for the manually nested route.
