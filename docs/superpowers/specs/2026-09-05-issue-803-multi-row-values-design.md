...
3. `true`는 최대 허용 행 수 + 1개까지만 수집한다. 빈 입력은 DB 접근 없이 빈 결과를 반환한다. 한도는 현재 트랜잭션의 방언을 따른다.
...
7. 반환값은 Exposed가 반환한 `ResultRow`만 매핑한다. PostgreSQL의 부분 충돌 결과·순서를 검증한다. H2의 고유키 ignore 미지원, Oracle/MySQL의 generated-key 제한은 성공으로 취급하지 않고 문서화한다. `saveAll`은 생성 ID 계약을 그대로 유지한다.
...
11. PostgreSQL 부분 충돌 검증은 반환된 각 ID로 조회한 실제 행과 반환 엔티티가 일치하고 충돌 행을 포함하지 않는지 검사한다. 결과 개수는 입력과 다를 수 있으므로 `zip` 기반 대응을 보장하지 않는다. 일반 입력의 순서를 테스트하되 모든 dialect의 순서를 일반화하지 않는다.
...
| PostgreSQL, ignore=true | 실제 삽입 행만 반환하고 ID별 조회 결과와 일치 검증; input.zip(result) 금지 |
| H2, ignore=true | native 고유키 ignore 지원을 보장하지 않음; SQL 미지원 오류는 그대로 전파하며 false로 자동 전환하지 않음 |
...
- generated ID, 입력 순서, PostgreSQL 부분 충돌 반환값 및 writer 중복 오류 후 rollback을 검증한다.
