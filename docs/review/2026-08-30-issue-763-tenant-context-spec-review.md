...
| P0 | operator/Ops | 없음. 두 모듈을 선택형으로 등록하고 CI, ABI, dependency boundary, manual inventory를 함께 검증한다. | 없음 | N/A |
...
P0=0, P1=0이다. P2는 외부 catalog handoff 상태를 명세에 이미 기록한
잔여 리스크이며 구현을 막지 않는다. JDBC·R2DBC adapter는 기존 helper의
